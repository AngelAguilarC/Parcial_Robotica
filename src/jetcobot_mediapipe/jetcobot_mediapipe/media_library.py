...
from jetcobot_msgs.msg import ArmJoint, ImageMsg
...
class Media_ROS(Node):
    def __init__(self):
        ...
        self.pub_image = self.create_publisher(ImageMsg, 'Media_image', 10)
    ...
    def pub_arm(self, joints, id=6, angle=180, runtime=500):
        ...
        # self.get_logger().info(str(arm_joint))
    ...
    def cancel(self):
        self.destroy_publisher(self.pubCmdVel)
        self.destroy_publisher(self.pub_image)
        self.destroy_publisher(self.pubBuzzer)
        self.destroy_publisher(self.pubPoint)
        self.destroy_subscription(self.sub_JoyState)
